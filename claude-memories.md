...
- **Phase 3 (/impact) contract notes (2026-07-06, from final Phase 2 review)**: analytics-repo is range-bound (7/30/90) — /impact needs a since-launch/getLifetimeStats variant, do NOT ship it on days=90. Chart wrappers are ssr:false → charts won't be in ISR'd HTML; decide if citable static markup matters. Login rate limit is per-serverless-instance (in-memory) — ADMIN_PASSWORD strength is the real prod defense; Upstash/Turso counter if it ever matters. Review mutations now have in-handler auth too (defense-in-depth, app/api/reviews/[id]/handler.ts). Deploy runbook: Vercel env needs ADMIN_PASSWORD + ADMIN_SESSION_SECRET; run db:init against prod Turso (legacy-safe).
- **Dashboard v2 Shipped (2026-07-06, branch feature/dashboard-v2)**: Comprehensive admin dashboard: Overview (6 KPIs w/ prev-period deltas, chart w/ dashed prev overlay, activity feed), Traffic (new/returning via all-time first_seen, top pages, countries from x-vercel-ip-* headers — null locally, leaderboard, practice funnel), AI Ops (per-model tokens + cost from lib/constants/model-prices.ts — MODEL PRICES MUST KEY THE DATED SNAPSHOT ID the API returns e.g. claude-haiku-4-5-20251001; unpriced model → totalCost null). events table REBUILT: no CHECK constraint (lib/constants/events.ts allowlist is sole gatekeeper), +country/city; migration transactional (begin/commit — executeMultiple is NOT atomic per-statement, proven empirically). practice_complete event wired to all 3 mode completions (means "reached results screen", not "passed"). SQLite datetime strings are UTC without T/Z — ALWAYS normalize to ISO-Z at the repo boundary before new Date() (activity.ts pattern). Known flake: prompt-improver.test.tsx one-shot fetch mock under parallel jest — passes in isolation. Owner's "2 Issues" dev overlay = Recharts defaultProps deprecation, cosmetic.
- **SHIPPED TO PROD (2026-07-07)**: main pushed 10df598→125f68d (109 commits: IA rebrand, analytics foundation, admin dashboard v2, OpenAI provider, dual-pillar copy). REVERT ANCHOR: tag `live-2026-07-07` = pre-rollout production (git checkout live-2026-07-07 -b rollback && push, or Vercel instant rollback). POST-DEPLOY REQUIRED (owner): 1) regenerate Turso auth token (old one 401s) + `TURSO_DATABASE_URL=<prod> TURSO_AUTH_TOKEN=<new> npm run db:init` — until then analytics writes silently fail on prod (missing columns; site UX unaffected, fire-and-forget); 2) Vercel env: ADMIN_PASSWORD + ADMIN_SESSION_SECRET (admin is fail-closed 503 until set); 3) optional: AI_PROVIDER=openai + AI_DEFAULT_MODEL=gpt-5.4-nano-2026-03-17 + OPENAI_API_KEY in Vercel (unset → falls back to anthropic provider, needs ANTHROPIC_API_KEY there); 4) optional GA: NEXT_PUBLIC_GA_ID + NEXT_PUBLIC_ENABLE_ANALYTICS=true.
- **Reviews System Overhaul Shipped (2026-07-08, pushed 381b119→6bc0bf5)**: Fully first-party review loop: /reviews in-page form (replaced DEAD Google link with placeholder ID + "Coming Soon" emoji card) → POST /api/reviews → /admin/reviews moderation → homepage wall (components/reviews/approved-reviews-wall.tsx, hidden until MIN_REVIEWS_TO_SHOW=2 approved). Solicitation: post-vote ask in time-saved-feedback + prominent centered dialog for RETURNING visitors only (review-ask-popup.tsx, localStorage np-visited/np-review-asked, once ever, excluded on /admin+/reviews). Famewall/Senja REMOVED (iframe, CSP allowances, camera/mic permissions, privacy-policy entry). AUTHENTICITY PURGE: deleted fabricated sample-reviews.ts + hardcoded testimonials-grid ("Sarah Mitchell" etc.) + fake schema.org aggregateRating (4.8/2453!) from lib/seo/metadata.ts — NEVER fabricate reviews/ratings (FTC + USCIS petition risk); reintroduce aggregateRating only computed from real reviews table. Legal pages rewritten to describe actual data practices (privacy: np_anon/analytics/AI-inputs-to-providers/progress sync; terms: AI disclaimers). Footer: Robert Appiah attribution, dual-pillar copyright line. Owner still needs: migrate real Famewall testimonials via /reviews form, solicit AI-tool reviews.
- **HARD RULE (2026-07-08, caused two dev-server outages)**: NEVER `rm -rf .next` or `next build` in the project dir while a dev server is running — it deletes the running server's manifests and every page 500s (ENOENT routes-manifest.json) until restart. Any agent dispatch that includes build verification MUST either kill dev servers first (`pkill -f "next dev"`) and restart after, or skip the build step and let the controller run it. Include this warning verbatim in implementer prompts that verify builds.
